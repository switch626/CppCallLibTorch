...
void prepareImgTensor(std::vector<torch::Tensor>& img_tensor, std::vector<cv::Mat>& img_t, int index)
{
	...
	//torch::from_blob(img_float.data, { 1, input_image_size, input_image_size, 3 }).permute({ 0, 3, 1, 2 }).to(torch::kCUDA);
...
//
//auto img_tensor = torch::from_blob(img_float.data, { 1, input_image_size, input_image_size, 3 }).permute({ 0, 3, 1, 2 }).to(torch::kCUDA);
